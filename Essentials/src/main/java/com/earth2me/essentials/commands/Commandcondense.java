    ...
    private boolean condenseStack(final User user, final ItemStack stack, final boolean validateReverse) throws ChargeException, MaxMoneyException {
        ...
        if (condenseType != null) {
            ...
            if (amount > 0) {
                ...
                final List<ItemStack> remainingItems = condenseType.getRemainingItems();
                if (!remainingItems.isEmpty()) {
                    ...
                    if (remainingItemType == Material.GLASS_BOTTLE) {
                        new Trade(new ItemStack(remainingItemType, input.getAmount()), ess).pay(user, OverflowType.DROP);
                    }
                }
                return true;
            }
